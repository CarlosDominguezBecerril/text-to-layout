...
class SupervisedTrainer():
    ...
    def __init__(self, seq2seq, vocab, epochs, print_every, loss_class, loss_bbox_xy, loss_bbox_wh, 
    batch_size, hidden_size, max_lr, opt_func, steps_per_epoch, checkpoints_path="./checkpoints", 
    gaussian_dict=None, validator_output_path="./evaluator_output", save_output=False):
    ...
    def train_batch(self, batch_step, objs, triples, obj_to_img, triple_to_img, valid_objs, coco_boxes, coco_ids, coco_to_img):
        ...
        outputs = self.seq2seq(objs, triples, obj_to_img, triple_to_img, valid_objs, coco_boxes, coco_ids, coco_to_img)
        ...
        for di in range(trg_len-1): # -1 because we have one object less
    ...
    def train_epoches(self, train_loader, train_ds, val_loader, val_ds, start_epoch=0):
        ...
        for epoch in range(start_epoch, start_epoch + self.epochs):
            ...
            for batch in tqdm(train_loader):
                objs, triples, obj_to_img, triple_to_img, valid_objs, coco_boxes, coco_ids, coco_to_img, all_idx = batch
                lloss, bloss_wh, bloss_xy, bloss_xy_MSE = self.train_batch(step, objs, triples, obj_to_img, triple_to_img, valid_objs, coco_boxes, coco_ids, coco_to_img)
